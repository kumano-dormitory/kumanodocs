...
class TestMeeting(TestCase):
    ...
    def test_posting_table_meeting_queryset(self):
        assert len(Meeting.posting_table_meetings()) == 0
        Meeting(meeting_date=date.today() + timedelta(weeks=4)).save()
        assert len(Meeting.posting_table_meetings()) == 1
